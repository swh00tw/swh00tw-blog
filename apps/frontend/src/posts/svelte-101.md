...
- In React, for each functional component, we can only return one node. A popular workaround is wrapping the nodes inside `<>` or using React.Fragment. However, in Svelte, don't worry about this. You can return an arbitrary number of nodes.
